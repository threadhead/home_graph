...
See the following {perl script}[http://gist.github.com/151716] for sample of how to setup a daemon that can monitor an elk-m1 system using the {ELKM1 perl script}[https://github.com/threadhead/elkm1--control/tree].
...
This is pretty convoluted, but to log thermostat data I use Homeseer (version 1.7.X, don't laugh). Homeseer triggers a perl script, that in turn triggers a Ruby script. The ruby script then logs the data. Why all the complexity? Simple, Homeseer and ODBC just are not stable enough, but ruby is. Go figure?
